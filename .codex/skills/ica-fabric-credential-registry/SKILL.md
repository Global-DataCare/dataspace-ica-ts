...
- Keep organization verification OTP separate from professional invitation OTP.
  The invitation captures employee email and employee phone, and its initial
  functional channel may be employee email instead of SMS. Veterinary CA/US
  requires the prior official-marketplace or technical-controller voice proof.
  Human-health CA/US production requires completed physical-site verification:
  the organization remains inactive until the technical controller submits the
  numeric legal-address code bound to the hash of the PDF generated by the
  form. Only then may invitation delivery begin. That email OTP does not verify
  the organization and does not complete DCR. Runtime transport remains pending;
  follow
