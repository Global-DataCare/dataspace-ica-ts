# 01 Existing Deep Dives

This section indexes older detailed documents that are still useful.

## Identity and key management

- [`../organization-key-binding-v2.md`](../organization-key-binding-v2.md)
- [`../organization-key-management.md`](../organization-key-management.md)
- [`../organization-terms-remove-v2.md`](../organization-terms-remove-v2.md)

## Security and deployment

Environment-specific security, deployment and troubleshooting material belongs
in the operations documentation outside this source repository.

## Discovery and interoperability

- [`../adr-0002-discovery-catalog-profile.md`](../adr-0002-discovery-catalog-profile.md)
- [`../pontusx-integration.md`](../pontusx-integration.md)
- [`../oidc4ida-evidence-mapping.md`](../oidc4ida-evidence-mapping.md)

## ADRs and backlog notes

- [`../adr-0004-veterinary-controller-phone-otp-backlog.md`](../adr-0004-veterinary-controller-phone-otp-backlog.md)
- [`../adr-0005-professional-invitation-email-otp-backlog.md`](../adr-0005-professional-invitation-email-otp-backlog.md)

- [`../adr-0001-confidential-storage-model.md`](../adr-0001-confidential-storage-model.md)
- [`../adr-0003-key-rotation-and-vc-reissuance-backlog.md`](../adr-0003-key-rotation-and-vc-reissuance-backlog.md)
- [`../backend-auth-migration.md`](../backend-auth-migration.md)
