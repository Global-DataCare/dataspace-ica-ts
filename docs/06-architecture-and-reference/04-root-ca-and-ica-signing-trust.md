...
`did:web:ca.member.example` surface; the Root private key is never copied to ICA,
...
Accuro custody. A configured `ICA_VC_SIGNING_PRIVATE_KEY_PEM` takes precedence
over seed derivation.
...
`ICA_ROOT_CA_DID=did:web:ca.member.example` resolves that same Root public key as a
...
```dotenv
ICA_ROOT_CA_DID=did:web:ca.member.example
ICA_ROOT_CA_CERT_SHA256=<fingerprint from https://ca.member.example/.well-known/trust.json>
