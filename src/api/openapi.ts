      ...
      '/ica/cds-{jurisdiction}/v1/{sector}/{networkKind}/pdf/{resourceType}/_verify': {
        post: {
          ...
          description:
            ...
            + '- the actor binding key travels in `body.data[].resource.controller.publicKeyJwk`; in legacy registration it belongs to the legal representative\n'
            + '- optional flattened GW claims travel in `body.data[].resource.meta.claims`; entry-level `meta.claims` is legacy only\n'
            + '- ICA issues a third `ServiceControllerCredential` only when the signed controller is the representative or `controller.sameAs` explicitly matches the separately designated controller that owns the submitted JWK\n'
            + '- a different PDF controller email without that explicit key-owner match remains a hashed pending designation for later sector `_issue` and DCR\n'
            ...
            + '- `ICA_ALLOW_LEGACY_CONTRACT=true` is a deployment-only `SECURITY_MODE=compat` bridge: visible signed text may distinguish a labelled technical contact from an earlier same-domain representative, with a narrow first/last same-domain three-address fallback; ambiguous sequences fail closed, and `legalRepresentative.sameAs|email` remains only the final representative fallback\n'
            + '- signature, chain, revocation, template, organization identity and controller-key ownership checks remain unchanged; the bridge is disabled by default and ignored in strict mode\n\n'
      ...
      '/ica/cds-{jurisdiction}/v1/{sector}/{networkKind}/pdf/{resourceType}/_verify-response': {
        post: {
          ...
          description:
            ...
            + '- a legacy two-entry result binds the submitted JWK only to the representative and may retain a different signed controller as `designatedControllerSameAs` pending its own sector binding\n'
            ...
            + '- legacy contracts may use `ICA_ALLOW_LEGACY_CONTRACT=true` only with `SECURITY_MODE=compat`; it first extracts distinct representative and technical-controller identities from visible signed text, fails closed on ambiguous order, and uses the request only as a final representative fallback without weakening PDF verification; it is disabled by default\n\n'
            + '**SDK v2**\n'
