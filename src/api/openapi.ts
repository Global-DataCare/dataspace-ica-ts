      ...
      '/ica/cds-{jurisdiction}/v1/{sector}/{networkKind}/pdf/{resourceType}/_verify': {
        post: {
          ...
          description:
            ...
            + '- `ICA_ALLOW_LEGACY_CONTRACT=true` is a deployment-only `SECURITY_MODE=compat` bridge for contracts whose signed evidence lacks representative email: it may source only representative `sameAs` from `legalRepresentative.sameAs|email`; signature, chain, revocation, template and organization identity checks remain unchanged; disabled by default and ignored in strict mode\n\n'
      ...
      '/ica/cds-{jurisdiction}/v1/{sector}/{networkKind}/pdf/{resourceType}/_verify-response': {
        post: {
          ...
          description:
            ...
            + '- legacy contracts may use `ICA_ALLOW_LEGACY_CONTRACT=true` only with `SECURITY_MODE=compat`; it fills the missing representative identity from the request but does not weaken PDF verification, and it is disabled by default\n\n'
            + '**SDK v2**\n'
