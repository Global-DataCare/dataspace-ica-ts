...
export type CredentialRetrievePollOutcome =
  | { type: 'error'; statusCode: number; message: string }
  | { type: 'pending'; location: string; retryAfter: number }
  | { type: 'failed'; payload: unknown }
  | { type: 'succeeded'; payload: unknown };

function sameRoute(a: CredentialRetrieveRouteContext, b: CredentialRetrieveRouteContext): boolean {
  return (
...
function buildDidcommCredentialRetrieveMessage(
...
export class CredentialRetrieveResponseManager {
  private readonly jobStore: InMemoryEntityJobStore<CredentialRetrieveRouteContext, CredentialRetrieveResult>;

  constructor(jobStore: InMemoryEntityJobStore<CredentialRetrieveRouteContext, CredentialRetrieveResult>) {
  ...
  async poll(
    ...
    req: IncomingMessage,
    requestUrl: URL,
  ): Promise<CredentialRetrievePollOutcome> {
    const thid = await parsePollingThreadId(req, requestUrl);
    if (!thid) {
      return {
        type: 'error',
        statusCode: 400,
        message: 'Missing thid for _retrieve-response polling.',
      };
    }
    ...
    if (!job) {
      return {
        type: 'error',
        statusCode: 404,
        message: `Credential _retrieve job not found for thid=${thid}.`,
      };
    }
    ...
    if (job.status === 'failed') {
      const diagnostics = job.error || 'Unknown credential _retrieve failure.';
      ...
      const failedBody: VerifyBundleResponse = {
        ...
        data: [
          {
            type: 'NetworkCredentialRetrieve-v2.0',
            resource: {
              id: `urn:uuid:${randomUUID()}`,
              type: 'network-credential-retrieve-v2.0',
              thid,
      ...
      return {
        type: 'failed',
        payload: buildDidcommCredentialRetrieveMessage(route, thid, failedBody, req),
      };
    }
    ...
    if (!job.result?.item) {
      return {
        type: 'error',
        statusCode: 500,
        message: `Credential _retrieve job ${thid} finished without result payload.`,
      };
    }
    ...
    const outcome = buildOperationOutcome([
      {
        ...
        code: 'informational',
        diagnostics: 'Credential retrieve succeeded with one signed credential.',
      },
    ]);
    ...
    return {
      type: 'succeeded',
      payload: buildDidcommCredentialRetrieveMessage(route, thid, succeededBody, req),
    };
  }
}
